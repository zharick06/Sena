...
public class VentanaOperaciones extends JFrame implements ActionListener{
	...
	private void iniciarComponentes() {
		...
		btnButtonEdad = new JButton("\u00BFSoy mayor de edad?");
		btnButtonEdad.setFont(new Font("Tahoma", Font.PLAIN, 17));
		btnButtonEdad.addActionListener(this);
		btnButtonEdad.setBounds(171, 342, 233, 23);
		contentPane.add(btnButtonEdad);
		
		btnListaPersona = new JButton("Lista Persona");
	    ...
	    lblmayor = new JLabel();
		lblmayor.setBounds(171, 391, 233, 35);
		contentPane.add( lblmayor);
		
		scrollPane = new JScrollPane();
		scrollPane.setBounds(141, 484, 290, 124);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==btnListaPersona) {
			textArea.setText(misProcesos.imprimirUsuarios()+"");			
		}
		
		else if (e.getSource()==btnButtonEdad) {
			
			Usuarios miUsuario=new Usuarios();
			miUsuario.setNombre(textNombre.getText());
			try {
				int yearActual=Integer.parseInt(contAnoActual.getText());
			    int yearUser=Integer.parseInt(contAnoUsuario.getText());
			    int mountActual=Integer.parseInt(contMesActual.getText());
			    int mountUser=Integer.parseInt(contMesUsuario.getText());
			    int dayActual=Integer.parseInt(contDiaActual.getText());
			    int dayUser=Integer.parseInt(contDiaUsuario.getText());
			    int años= yearActual-yearUser;
			    int meses= mountActual-mountUser;
			    int dias=dayActual-dayUser;
				miUsuario.setEdad(años);
				misProcesos.guardarUsuario(miUsuario);
				lblmayor.setText(misProcesos.mayor(años,meses,dias));
